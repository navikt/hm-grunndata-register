...
package no.nav.hm.grunndata.register.runtime
...
class Where<T>(var root: Root<T>, var criteriaBuilder: CriteriaBuilder) {
    ...
    infix fun Expression<String>.like(other: Expression<String>) = addPredicateExpString(criteriaBuilder::like, other)
...
//  fun <Y : Comparable<Y>> Expression<out Y?>.between(x: Expression<out Y?>, y: Expression<out Y?>) = addPredicate(criteriaBuilder.between(this, x, y))
    ...
    private inline fun Expression<String>.addPredicateExpString(fn: (Expression<String>, Expression<String>) -> Predicate, value: Expression<String>) {
        addPredicate(fn.invoke(this, value))
    }
