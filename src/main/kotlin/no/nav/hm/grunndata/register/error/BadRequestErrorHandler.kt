...
class BadRequestErrorHandler : ExceptionHandler<BadRequestException, HttpResponse<ErrorMessage>> {
    override fun handle(
        ...
        val response =
            when (error.type) {
                ...
                ErrorType.CONFLICT ->
                    HttpResponseFactory.INSTANCE.status<ErrorMessage>(HttpStatus.CONFLICT)
    ...
    private fun createMessage(error: BadRequestException): ErrorMessage {
        if (error.message?.contains("articlenameseriesuuidunique") == true) {
    ...
    return when (error) {
        ...
        is InvalidFormatException ->
            HttpResponse
                .badRequest(
                    ErrorMessage(
                        "Invalid value: ${error.value} at ${error.pathReference}",
        ...
        is ValueInstantiationException ->
            HttpResponse.badRequest(
                ErrorMessage(
                    "Wrong value: ${error.message} at ${error.pathReference}",
                    ErrorType.INVALID_VALUE,
                ),
            )
